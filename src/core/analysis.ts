...
/** Ищем распрямление только до входа в следующий поворот: дальше карт вращается
 *  уже по другой причине. Если до него так и не распрямился — это не пропуск
 *  данных, а содержательный ответ: связка проходится одной непрерывной дугой. */

/** Сколько метров карт должен ехать прямо, чтобы это считалось выходом из поворота.
 *  Без этого условия метрика ловит первое же касание порога, а в шпильке это
 *  обычно секундный сброс дуги в середине: радиус раскрывается, тут же
 *  закрывается обратно, и «распрямился» получается на 30 м раньше правды. */
const UNWIND_HOLD = 15;
...
export function zoneStats(tr: LapTrace, zones: Zone[], grid: Float64Array): ZoneStats[] {
  ...
  return zones.map((z, zi) => {
    ...
    const hold = Math.round(UNWIND_HOLD / step);
    let win = zones[(zi + 1) % nz].corner.sStart - z.corner.sApex;
    if (win < 0) win += grid[N - 1] + step;
    let sUnwind = NaN;
    if (isFinite(tr.kap[ap])) {
      let run = 0;
      for (let d = 0; d * step < win; d++) {
        if (Math.abs(tr.kap[(ap + d) % N]) < 1 / STRAIGHT_R) {
          if (++run >= hold) { sUnwind = (d - hold + 1) * step; break; }
        } else run = 0;
      }
    }
