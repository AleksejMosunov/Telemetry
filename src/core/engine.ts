...
export function verdict(
  ...
  // Числа берём с краёв ряда: именно их сравнение и есть весь вывод.
  ...
  if (lo && hi) {
    return {
      ...
      why: `Карт отстаёт одинаково на любой скорости — ${pair}. Слабый мотор так себя не ведёт: `
        + 'на малой скорости тяги хватает обоим, и разница вылезала бы только на быстрых участках.',
      action: 'Смотреть надо на то, что мешает всегда: вес пилота, тугая ось и подшипники, притирающий тормоз.',
    };
  }
  if (hi) {
    return {
      ...base, kind: 'top',
      title: 'Похоже на мотор',
      why: `Чем быстрее едет карт, тем сильнее он отстаёт — ${pair}. Это почерк слабого мотора: `
        + 'внизу тяги хватает обоим, а наверху не хватает лошадей.',
      action: 'Проверять мотор: выхлоп, зажигание, компрессию, передаточное отношение.',
    };
  }
  if (lo) {
    return {
      ...base, kind: 'bottom',
      title: 'Похоже на низы мотора',
      why: `Карт отстаёт на выходе из медленных поворотов и догоняет на быстрых — ${pair}. `
        + 'Значит не хватает не мощности, а подхвата с низов.',
      action: 'Проверять карбюратор и сцепление.',
    };
  }
  return {
    ...base, kind: 'mixed',
    title: 'Однозначного ответа нет',
    why: `Карт отстаёт, но по-разному в разных местах — ${pair}, без общей картины.`,
    action: 'Нужно больше кругов или заезд на той же трассе с теми же картами.',
  };
}
