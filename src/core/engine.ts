...
export type VerdictKind = 'none' | 'top' | 'bottom' | 'all' | 'mixed' | 'weak';

/**
 * Насколько отставание слабого мотора должно вырасти между двумя скоростями.
 *
 * Нужно не для ответа, а чтобы понять, есть ли у теста разрешающая способность.
 * Мотор отдаёт мощность, а не силу: тяга падает как P/v, а сопротивление растёт,
 * поэтому одна и та же нехватка мощности стоит тем дороже, чем быстрее едет карт.
 * Разница между «слабый мотор» и «лишняя масса» — это как раз скорость роста, и
 * если ворота стоят близко друг к другу, роста не хватит, чтобы вылезти из
 * погрешности. Тогда честный ответ — «не определить», а не «значит масса».
 *
 * Числа взяты для прокатного карта: ~10 кВт на колесе, 175 кг с пилотом,
 * CdA·½ρ ≈ 0.42, сопротивление качению 1.5%. Точность тут не важна — важен
 * порядок роста, а он от этих чисел почти не зависит.
 */
function engineRise(kmh1: number, kmh2: number, pct: number): number {
  const share = (kmh: number) => {
    const v = kmh / 3.6;
    return Math.min(0.8, (0.42 * v ** 3 + 25.7 * v) / 9700);
  };
  const k = (kmh: number) => 1 / (1 - share(kmh));
  return pct * (k(kmh2) / k(kmh1) - 1);
}
...
export function verdict(
  ...
  // разницу, — карточка сама себе противоречила: сверху «карт лучше», внизу
  // «сопоставимо с шумом».
  const meanErr = Math.sqrt(points.reduce((s, p) => s + p.err ** 2, 0)) / points.length;
  if (Math.abs(pct) <= meanErr) {
    return {
      ...base, kind: 'none',
      title: 'Карты едут одинаково',
      why: `Разница в разгоне ${pct >= 0 ? '+' : '−'}${Math.abs(pct).toFixed(1)}% — меньше собственной `
        + `погрешности замера (±${meanErr.toFixed(1)}%). На таких данных это ноль.`,
      action: '',
    };
  }
  if (pct < 0) {
    return {
      ...
      why: `Он проходит те же ворота короче на ${Math.abs(pct).toFixed(1)}% при погрешности `
        + `±${meanErr.toFixed(1)}%.`,
      action: '',
    };
  }
  ...
  // отсутствие наклона доказательством невиновности мотора нельзя.
  const rise = engineRise(a.mid, b.mid, pct);
  if (Math.abs(change) <= err && Math.abs(rise) < err) {
    return {
      ...base, kind: 'weak',
      ...
      action: 'Нужен заезд там, где карт разгоняется в широком диапазоне — от медленного поворота до '
        + 'длинной прямой. Либо взвесить пилотов: если вес равный, остаётся мотор или лишнее трение.',
    };
  }
  ...
  if (change < -err) {
    return {
      ...
      why: `Больше всего карт теряет на медленных разгонах, на быстрых — заметно меньше: ${pair}. `
        + 'Догнать он нигде не догоняет, но мощности ему хватает — не хватает подхвата сразу после '
        + 'поворота.',
  ...
  return {
    ...
    why: `Отставание не зависит от скорости: ${pair}, и разница между ними меньше погрешности замера `
      + `(±${err.toFixed(1)}%). Будь виноват мотор, отставание выросло бы примерно на `
      + `${Math.abs(rise).toFixed(1)}% — это заметно больше погрешности, так что рост бы увидели.`,
